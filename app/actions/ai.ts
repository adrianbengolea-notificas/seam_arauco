...
import { runTranscribePlanillaObservaciones } from "@/lib/ai/flows/transcribe-planilla-observaciones";
import {
...
const transcribePlanillaSchema = z.object({
  audioDataUrl: z
    .string()
    .regex(/^data:audio\/[a-z0-9.+-]+;base64,/i, "Audio inválido")
    .max(9_500_000),
  otN: z.string().min(1).max(64),
  assetLabel: z.string().min(1).max(500),
});

/** Transcribe audio + redacción tipo observaciones (Gemini multimodal). */
export async function actionTranscribePlanillaObservacionesAudio(
  idToken: string,
  input: z.infer<typeof transcribePlanillaSchema>,
): Promise<ActionResult<{ text: string }>> {
  return wrap(async () => {
    const profile = await requirePermisoFromToken(idToken, "historial:informe_ia");
    const cfg = await getCentroConfigMergedCached(profile.centro);
    ...
    const parsed = transcribePlanillaSchema.parse(input);
    const text = await runTranscribePlanillaObservaciones(parsed);
    return { text };
  });
}
