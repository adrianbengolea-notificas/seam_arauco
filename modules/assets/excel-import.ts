...
/**
 * Inferencia por fila: corrige especialidad cuando la hoja (p. ej. GG) agrupa equipos pero la
 * denominación describe aire acondicionado / HVAC.
 */
export function inferEspecialidadPredeterminada(
  denominacion: string,
  codigo_nuevo: string,
  desdeHoja: EspecialidadActivo,
): EspecialidadActivo {
  const t = normHeader(`${denominacion} ${codigo_nuevo}`);
  if (!t) return desdeHoja;

  // Evitar clasificar instalaciones de aire comprimido / compresores como AA
  if (/\bcompresor(es)?\b/.test(t) || /\baire\s+comprimido\b/.test(t) || /\bcompressed\s+air\b/.test(t)) {
    return desdeHoja;
  }

  const looksAA =
    /^aire\b/.test(t) ||
    /\baire\s+acond/.test(t) ||
    /\bacondicionad/.test(t) ||
    /\bsplit\b/.test(t) ||
    /\bchiller\b/.test(t) ||
    /\bvrf\b/.test(t) ||
    /\bfancoil\b/.test(t) ||
    /\bfan[\s-]?coil\b/.test(t) ||
    /\bclimatiz/.test(t) ||
    /\bunidad\s+(exterior|interior)\b/.test(t) ||
    /\btorre\s+de\s+enfri/.test(t) ||
    /\bevaporativ/.test(t);

  if (looksAA) return "AA";

  return desdeHoja;
}

/** Reconocimiento por nombre de pestaña — solo las hojas que matchean se importan (resto omitidas). */
function sheetEspecialidad(sheetName: string): EspecialidadActivo | null {
  const n = normHeader(sheetName);
  if (!n) return null;

  const isGG =
    (n.includes("grupo") && n.includes("generador")) ||
    (n.includes("grupo") && n.includes("generad")) ||
    /\bgg\b/.test(n) ||
    /\bgeneradores?\b/.test(n);

  const isHG = /\bhg\b/.test(n) || n.includes("higienico") || n.includes("gas natural");

  const isElectric =
    n.includes("electrico") ||
    /\belectrico(s)?\b/.test(n) ||
    (n.includes("tablero") && n.includes("elect"));

  const isAA =
    (n.includes("aire") && (n.includes("acondicionado") || n.includes("acond"))) ||
    /\baa\b/.test(n) ||
    /\b(aire[-_\s]?acond)/.test(n) ||
    /\b(hvac|vrf)\b/.test(n) ||
    n.includes("climatizacion") ||
    n.includes("refrigeracion") ||
    n.includes("splits");

  // Orden: especialidades específicas antes que GG (a veces hay solapamiento de palabras).
  if (isAA && !isGG) return "AA";
  if (isElectric) return "ELECTRICO";
  if (isHG) return "HG";
  if (isGG) return "GG";
  if (isAA) return "AA";
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    for (let r = hr + 1; r < matrix.length; r++) {
      ...
      rows.push({
        ...
        centro,
        especialidad_predeterminada: inferEspecialidadPredeterminada(
          denominacion || codigo_nuevo,
          codigo_nuevo,
          esp,
        ),
