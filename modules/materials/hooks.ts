...
function applyMaterialesFilters(rows: MaterialOTConsumoRow[], f: MaterialesOTFilters): MaterialOTConsumoRow[] {
  ...
  if (f.especialidad && f.especialidad !== "todos") {
    const want: NonNullable<MaterialOTConsumoRow["otEspecialidad"]> =
      f.especialidad === "A"
        ? "AA"
        : f.especialidad === "E"
          ? "ELECTRICO"
...
export function useMaterialsCatalogLive(max: number = 500): {
  ...
  itemsSinMinimo: MaterialCatalogItem[];
  loading: boolean;
  error: Error | null;
} {
  ...
  const itemsSinMinimo = useMemo(() => {
    return items.filter(
      (it) =>
        it.activo !== false &&
        (it.stock_minimo == null || it.stock_minimo === undefined),
    );
  }, [items]);

  return { items, itemsBajoStock, itemsSinMinimo, loading, error };
}
