      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
            Reporting de materiales cargados en OTs (solo lectura). Período:{" "}
      ...
      {error ? (
        <p className="text-sm text-red-600">
          {(error as { code?: string }).code === "permission-denied"
            ? "No tenés permiso para leer materiales de OTs o las reglas de Firestore aún no están actualizadas. Si el problema continúa, pedí al admin que despliegue las reglas (`materiales_ot` / collection group)."
        ...
        <Card>
          ...
          <CardContent className="h-28">
            {pieTipo.length > 0 ? (
              ...
              <p className="text-sm text-zinc-500">Sin datos con tipo de OT</p>
            )}
          </CardContent>
        </Card>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2 lg:gap-3">
          ...
          <label className="flex min-w-[10rem] flex-1 flex-col gap-0.5 text-[10px] font-medium uppercase tracking-wide text-zinc-500 sm:min-w-[12rem]">
            ...
            <Input
              ...
              placeholder={profile?.centro ? `Ej. ${nombreCentro(profile.centro)}` : "Vacío = todos"}
              value={centro}
              onChange={(e) => {
        ...
        {authLoading || loading ? (
          ...
          <div className="overflow-x-auto rounded-lg border border-zinc-200 dark:border-zinc-800">
            <table
              ...
              aria-label="Materiales consumidos: detalle por ítem y OT"
            >
              <thead>
                <tr className="border-b border-zinc-200 bg-zinc-50 text-left text-xs font-semibold uppercase tracking-wide text-zinc-600 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-400">
                  ...
                  <th className="px-3 py-2">N.º orden servicio</th>
