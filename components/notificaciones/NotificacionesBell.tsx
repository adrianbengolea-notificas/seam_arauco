...
/** Destino al hacer clic: `href` / `otId`, o ruta embebida en el cuerpo (notif. antiguas sin href). */
function rutaNotificacion(n: {
  ...
  if (n.otId?.trim()) return `/tareas/${n.otId.trim()}`;
  if (n.tipo === "propuesta_disponible") {
    const m = n.cuerpo.match(/\/programa\/aprobacion\?semana=[^\s.]+/);
    if (m) return m[0];
  }
  return null;
}
...
export function NotificacionesBell() {
  ...
  const { itemsPendientes, noLeidas, loading, eliminarNotificacion, eliminarTodasPendientes } =
      ...
      {abierto ? (
        <div className="absolute right-0 z-50 mt-2 w-[min(100vw-2rem,22rem)] rounded-xl border border-white/15 bg-[var(--header-bg)] py-2 shadow-xl ring-1 ring-black/10">
          ...
          <div className="max-h-80 overflow-y-auto">
            {!itemsPendientes.length && !loading ? (
              <p className="px-3 py-6 text-center text-sm text-header-muted">Todo al día</p>
            ) : (
              <ul className="divide-y divide-white/10">
                {itemsPendientes.map((n) => {
                  const destino = rutaNotificacion(n);
                  return (
                  <li key={n.id}>
                    <button
                      type="button"
          ...
          {itemsPendientes.length ? (
            <div className="border-t border-white/10 px-2 pt-2">
              <Button
                ...
                onClick={() => eliminarTodasPendientes()}
              >
                Borrar todas
              </Button>
            </div>
