...
import { tienePermiso } from "@/lib/permisos/index";
...
const links: NavLink[] = [
  ...
  { href: "/programa", label: "📅 Programa de la semana", module: null },
  { href: "/materiales", label: "📦 Materiales", module: "materiales" },
  { href: "/activos", label: "🏭 Activos", module: "activos" },
  { href: "/activos/escaner", label: "📷 Escanear QR", module: "activos" },
];
...
export function AppSidebar({ mobileOpen, onNavigate }: AppSidebarProps) {
  ...
  const navLinks = useMemo(() => {
    const modulos = centroConfig?.modulos ?? { materiales: true, activos: true };
    let list = links.filter((l) => {
      if (l.href === "/activos/escaner" && !tienePermiso(rol, "activos:escanear_qr")) return false;
      if (l.module === "materiales") return modulos.materiales;
      if (l.module === "activos") return modulos.activos;
      return true;
    });
  ...
  const mainLinksNav = useMemo(() => {
    ...
    return ["/dashboard", "/tareas", "/programa"].map((h) => byHref[h]).filter((l): l is NavLink => Boolean(l));
  }, [mainLinks, rol]);
  const moduleLinks = navLinks.filter((l) => !mainHrefSet.has(l.href));
  const moduleSectionTitle =
    moduleLinks.length === 1 && moduleLinks[0]?.href === "/activos/escaner" ? "Activos" : "Módulos";
      ...
      >
        ...
        <div className="space-y-1">
          ...
          {(puede("programa:ver_calendario_anual") || puede("programa:ver_vencimientos_sa")) ? (
            ...
            >
              📋 Planes preventivos (calendario + vencimientos)
            </Link>
          ) : null}
          <PermisoGuard permiso="reportes:ver_cumplimiento">
            <Link
              href="/reportes/cumplimiento"
              className={itemClass(Boolean(pathname?.startsWith("/reportes/cumplimiento")))}
            >
        ...
        {showSistemaNav ? (
          <div className="mt-4 space-y-1 border-t border-white/10 pt-4">
            ...
            {(puede("admin:gestionar_usuarios") || puede("admin:cargar_programa")) ? (
              <Link
                href="/superadmin"
                className={adminItemClass(Boolean(pathname?.startsWith("/superadmin")))}
              >
