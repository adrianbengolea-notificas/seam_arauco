...
/** Referencia Admin a `work_orders/{id}` (p. ej. transacciones). */
export function adminWorkOrderRef(workOrderId: string) {
  return woRef(workOrderId);
}

export function newWorkOrderDocId(): string {
  return getAdminDb().collection(WORK_ORDERS_COLLECTION).doc().id;
}
