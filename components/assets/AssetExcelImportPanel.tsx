...
type AssetExcelImportPanelProps = {
  /** Al elegir planta para el Excel, también aplica el mismo valor al filtro «Centro» del listado. */
  onListCentroChange?: (centro: string) => void;
};

export function AssetExcelImportPanel({ onListCentroChange }: AssetExcelImportPanelProps = {}) {
  ...
  const applyCentroPerfil = useCallback(() => {
    ...
    if (isCentroInKnownList(c)) {
      ...
      setCentroLista(c);
      setCentroOtro("");
      onListCentroChange?.(c);
    } else {
      setCentroMode("otro");
      setCentroOtro(c);
      onListCentroChange?.(c);
    }
  }, [profile?.centro, onListCentroChange]);
  ...
  async function onFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > MAX_EXCEL_IMPORT_BYTES) {
      setMessage("Archivo muy grande (máximo 20 MB).");
      return;
    }
    const sec =
      effectiveSector ||
    ...
    <Card className="border-amber-200/80 dark:border-amber-900/50">
      <CardHeader>
        ...
        <CardDescription className="text-balance">
          ...
          variables públicas; si no está tu código, usá «Otro»). Al cambiar la planta aquí también se aplica el
          filtro «Centro» en el listado de abajo. Mismo formato que el script: AA y GG. Si el Excel trae «Centro» o
          «Planta», ese valor manda fila a fila.
        </CardDescription>
      </CardHeader>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (v === OTRO_CENTRO_VALUE) {
                  const nextOtro = centroOtro.trim() ? centroOtro : centroLista;
                  setCentroMode("otro");
                  setCentroOtro(nextOtro);
                  onListCentroChange?.(nextOtro.trim());
                } else {
                  setCentroMode("lista");
                  setCentroLista(v);
                  onListCentroChange?.(v.trim());
                }
              }}
            >
              {KNOWN_CENTROS.map((c) => (
