...
export default function ActivosPage() {
  ...
  const centrosOptions = useMemo(() => {
    ...
    const filtro = centro.trim();
    if (filtro) set.add(filtro);
  ...
  const filtered = useMemo(() => {
    ...
    return assets.filter((a) => {
      if (centro && (a.centro?.trim() || "") !== centro) return false;
  ...
  return (
    <div className="space-y-8">
      ...
      {!sessionLoading && canCreateAsset ? (
        <AssetExcelImportPanel onListCentroChange={setCentro} />
      ) : null}
      <Card>
        <CardHeader>
              ...
              <li key={a.id} className="text-sm transition-colors first:pt-0 last:pb-0">
                ...
                >
                  ...
                  <div className="mt-1 text-xs text-muted">
                    {a.ubicacion_tecnica} · {nombreCentro(a.centro ?? "")}
                  </div>
                </Link>
              </li>
            ))}
