...
/** Otra orden del mismo mantenimiento (misma clave lógica) aún sin cerrar — aviso SAP nuevo vs orden vieja. */
export type AntecesorOrdenAbierta = {
  aviso_id: string;
  n_aviso: string;
...
export type Aviso = {
  ...
  /** Hash estable (UT+frecuencia+especialidad+tipo) para cadena de avisos SAP. */
  clave_mantenimiento?: string;
  /** Si SAP mandó un aviso nuevo pero la orden anterior del mismo mantenimiento sigue abierta. */
  antecesor_orden_abierta?: AntecesorOrdenAbierta;
  texto_corto: string;
  texto_largo?: string;
  ...
  /** Meses del año en que debe ejecutarse (1=enero…12=diciembre). Leído de columnas mes del Excel. */
