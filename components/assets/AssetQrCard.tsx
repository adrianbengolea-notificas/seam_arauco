...
import { resolvePublicAppOrigin } from "@/lib/config/app-config";
...
export function AssetQrCard({ asset }: AssetQrCardProps) {
  ...
  useEffect(() => {
    ...
    if (origin) setQrValue(`${origin}/activos/${asset.id}`);
  }, [asset.id]);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Escaneá para abrir la ficha.</CardDescription>
      </CardHeader>
      <CardContent>
