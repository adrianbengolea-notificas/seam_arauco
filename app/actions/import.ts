 ...
 * `centroForzado`: si se informa, sobreescribe la derivación automática de centro para
 * todos los avisos del lote (útil cuando el Excel no tiene columna centro o el prefijo
 * UT es ambiguo, p. ej. BOSS→PF01 vs PM02).
 */
export async function confirmarImportAvisos(
  ...
  return wrap(async () => {
    ...
    const commit = await commitParsedAvisoRows({
      ...
      centroForzado: centroForzado?.trim() || undefined,
    });
