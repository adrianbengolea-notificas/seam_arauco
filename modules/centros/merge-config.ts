...
export function mergeCentroConfig(raw: CentroFirestoreDoc | Record<string, unknown> | undefined): CentroConfigEffective {
  ...
  /** Por defecto true si el documento no define el flag (centros sin migrar). */
  const requiere_firma_usuario_cierre =
    typeof data?.requiere_firma_usuario_cierre === "boolean" ? data.requiere_firma_usuario_cierre : true;
