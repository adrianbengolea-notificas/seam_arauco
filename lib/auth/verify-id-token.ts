...
function messageForVerifyIdTokenFailure(e: unknown): string {
  ...
  if (
    low.includes("invalid_rapt") ||
    (low.includes("invalid_grant") && low.includes("reauth")) ||
    low.includes("reauth related error")
  ) {
    return "Las credenciales de Google del servidor (Application Default Credentials) requieren reautenticación o no sirven para Firebase Admin. En desarrollo, lo más estable es usar cuenta de servicio: en `.env.local` definí `GOOGLE_APPLICATION_CREDENTIALS` con la ruta al JSON o `FIREBASE_SERVICE_ACCOUNT_KEY` (Firebase Console → Cuentas de servicio). Si usás solo `gcloud auth application-default login`, volvé a ejecutarlo y completá el login en el navegador, o pasá a cuenta de servicio. Ver `npm run diag:firebase`.";
  }
