...
type SuperadminPlanillasClientProps = {
  /** Dentro de Administración (vista anidada): sin cabecera propia ni enlace «volver». */
  embedded?: boolean;
};
...
export function SuperadminPlanillasClient({ embedded = false }: SuperadminPlanillasClientProps) {
  ...
  if (error) {
    return (
      <p className="text-sm text-red-600">Error de perfil: {mensajeErrorFirebaseParaUsuario(error)}</p>
    );
  }
  ...
  const introPautas = (
    <>
      Definiciones en{" "}
      <span className="font-mono text-xs">{COLLECTIONS.planilla_templates}</span> (GG, Elec, AA, Correctivos). Editá y
      guardá; los cambios aplican en tiempo real para nuevas planillas.
    </>
  );

  const planillasAyudaPanel = (
    <HelpIconTooltip ariaLabel="Ayuda: planillas digitales" panelClassName="left-0 right-auto sm:left-auto sm:right-0">
      <span className="block font-medium">Plantillas digitales</span>
      <span className="mt-1.5 block text-zinc-600 dark:text-zinc-400">
        Acá se administran las <strong className="font-medium text-foreground">estructuras</strong> de las planillas
        (secciones, ítems, grillas, etc.) que se usan al cargar trabajo en campo.
      </span>
      <span className="mt-2 block text-zinc-600 dark:text-zinc-400">
        Los modelos viven en Firestore en{" "}
        <code className="rounded bg-zinc-100 px-0.5 dark:bg-zinc-800">{COLLECTIONS.planilla_templates}</code>. Al
        guardar desde el editor, quien tenga la lista abierta ve el cambio al toque; las{" "}
        <strong className="font-medium text-foreground">nuevas</strong> instancias de planilla usan la versión actual.
      </span>
      <ul className="mt-2 list-disc space-y-1 pl-4 text-zinc-600 dark:text-zinc-400">
        <li>
          <strong className="font-medium text-zinc-800 dark:text-zinc-200">Expandir plantilla</strong>: ver secciones e
          ítems sin editar.
        </li>
        <li>
          <strong className="font-medium text-zinc-800 dark:text-zinc-200">Editar</strong>: modificar la definición y
          guardar en Firestore.
        </li>
        <li>
          <strong className="font-medium text-zinc-800 dark:text-zinc-200">Lista vacía</strong>: en desarrollo podés
          cargar datos con <code className="rounded bg-zinc-100 px-0.5 dark:bg-zinc-800">npm run seed:templates</code>.
        </li>
      </ul>
    </HelpIconTooltip>
  );
    ...
    >
      {embedded ? (
        <div className="flex items-start gap-2">
          <p className="min-w-0 flex-1 text-sm text-zinc-600 dark:text-zinc-400">{introPautas}</p>
          {planillasAyudaPanel}
        </div>
      ) : (
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <p className="text-sm text-zinc-600 dark:text-zinc-400">{introPautas}</p>
          </div>
          <Button asChild variant="outline" size="sm" className="shrink-0">
            <Link href="/superadmin">← Administración</Link>
          </Button>
        </div>
      )}
      ...
      {tplError ? (
        <p className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-800 dark:border-red-900 dark:bg-red-950/40 dark:text-red-200">
          No se pudieron cargar las plantillas: {mensajeErrorFirebaseParaUsuario(tplError)}
        </p>
      ) : null}
