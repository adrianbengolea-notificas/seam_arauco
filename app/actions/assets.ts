...
const assetFieldsSchema = z.object({
  ...
  especialidad_predeterminada: z
    .enum(["", "AA", "ELECTRICO", "GG", "HG"])
    .transform((v) => (v === "" ? undefined : v)),
  activo_operativo: z.boolean(),
  // Datos técnicos GG
  gg_motor_marca: z.string().trim().max(200).default(""),
  gg_motor_modelo: z.string().trim().max(200).default(""),
  gg_motor_serie: z.string().trim().max(200).default(""),
  gg_gen_marca: z.string().trim().max(200).default(""),
  gg_gen_modelo: z.string().trim().max(200).default(""),
  gg_gen_serie: z.string().trim().max(200).default(""),
  gg_gen_kva: z.string().trim().max(50).default(""),
  gg_combustible: z.string().trim().max(200).default(""),
});
...
export async function actionUpdateAsset(
  ...
  return wrap(async () => {
    ...
    await adminUpdateAsset(input.assetId, {
      ...
      ...(input.gg_motor_marca ? { gg_motor_marca: input.gg_motor_marca } : {}),
      ...(input.gg_motor_modelo ? { gg_motor_modelo: input.gg_motor_modelo } : {}),
      ...(input.gg_motor_serie ? { gg_motor_serie: input.gg_motor_serie } : {}),
      ...(input.gg_gen_marca ? { gg_gen_marca: input.gg_gen_marca } : {}),
      ...(input.gg_gen_modelo ? { gg_gen_modelo: input.gg_gen_modelo } : {}),
      ...(input.gg_gen_serie ? { gg_gen_serie: input.gg_gen_serie } : {}),
      ...(input.gg_gen_kva ? { gg_gen_kva: input.gg_gen_kva } : {}),
      ...(input.gg_combustible ? { gg_combustible: input.gg_combustible } : {}),
    });
