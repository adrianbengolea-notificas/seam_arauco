...
function ensurePuedeVerOt(session: UserProfileWithUid, wo: WorkOrder): void {
  ...
  if (r === "tecnico") {
    if (wo.tecnico_asignado_uid !== session.uid) {
      throw new AppError("FORBIDDEN", "Solo comentarios sobre tus OT asignadas");
    }
  }
}
...
export async function agregarComentario(
  ...
  return wrap(async () => {
    ...
    if (!wo) throw new AppError("NOT_FOUND", "Orden de trabajo no encontrada");
    ...
    crearNotificacionSeguro(destinatarios, {
      ...
      titulo: `Nuevo comentario en OT #${wo.n_ot}`,
      ...
      if (original && original.autorId !== session.uid) {
        crearNotificacionSeguro(
          [{ uid: original.autorId, rol: original.autorRol, centro: wo.centro }],
          {
            ...
            titulo: `Respondieron tu comentario en OT #${wo.n_ot}`,
            cuerpo: data.texto.trim().slice(0, 280),
            otId,
          },
        );
