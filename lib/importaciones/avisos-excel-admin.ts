...
async function loadUbicacionToAssetLookup(): Promise<UbicacionAssetLookup> {
  ...
  return { utToAsset, codigoNuevoByAssetId };
}
...
export async function importarAvisosDesdeExcelBuffer(input: {
  ...
  if (
    ...
    for (const sheetName of wb.SheetNames) {
      ...
      for (let r = hr + 1; r < matrix.length; r++) {
        ...
        const codigoEq = codigoNuevoByAssetId.get(assetId);
  ...
  } else if (input.modo === "calendario_mensual" || input.modo === "calendario_trimestral") {
    advertencias.push(
      "El calendario M/T (solo meses_programados) se importa desde Administración → Configuración e importación (vista previa + confirmar), no por este endpoint legacy.",
    );
