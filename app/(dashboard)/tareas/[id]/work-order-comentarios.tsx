...
export function WorkOrderComentariosSection({ otId, viewerUid, puedeComentar, esCliente }: Props) {
  ...
  const enviar = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setMsg(mensajeErrorFirebaseParaUsuario({ message: res.error.message }));
        return;
      }
      setTexto("");
      setRespondiendoA(undefined);
    } catch (e) {
      setMsg(mensajeErrorFirebaseParaUsuario(e));
    } finally {
      setEnviando(false);
    }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {loading ? "Cargando…" : error ? mensajeErrorFirebaseParaUsuario(error) : `${comentarios.length} mensajes`}
        </CardDescription>
      </CardHeader>
